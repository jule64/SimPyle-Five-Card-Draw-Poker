...
class Hand:
    def __init__(self, deck):
        ...
        self.images = [tkinter.PhotoImage(file=os.path.join(card_dir,self.cards[i]+'.gif')) for i in range(5)]

    def discard(self, deck, positions):
        for p in positions:
            self.cards[p] = deck.draw_card()
            self.images[p] = tkinter.PhotoImage(file=os.path.join(card_dir,self.cards[p]+'.gif'))
